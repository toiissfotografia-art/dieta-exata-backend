...
public class PaymentController {
    ...
    public ResponseEntity<?> gerarPix(@RequestBody Map<String, String> dados) {
        ...
        // Recupera o plano e o valor enviados pelo frontend (pix.html)
        String plano = Optional.ofNullable(dados.get("plano")).orElse("BRONZE").toUpperCase();
        
        // Lógica de valores baseada na sua seleção visual do HTML
        BigDecimal valor;
        try {
            ...
            valor = new BigDecimal("19.99"); // Fallback de segurança
        }
    ...
    public ResponseEntity<?> receberNotificacao(@RequestBody Map<String, Object> payload) {
        try {
            ...
            // Webhook sempre deve retornar 200 para o Mercado Pago não reenviar
            return ResponseEntity.ok().build(); 
        }
    }
